...
- **Internet to password-protected deployment edge** — the deployment is protected by Replit's password gate, so unauthenticated internet traffic should be stopped before it reaches Express. App-level auth and authorization still matter because anyone with the deployment password can reach the application surface.
