...
- **Application secrets and infrastructure access** — `DATABASE_URL`, `SESSION_SECRET`, Anthropic integration credentials, OIDC client credentials, `SQUARE_ACCESS_TOKEN`, `SQUARE_WEBHOOK_SIGNATURE_KEY`, and deployment configuration. Compromise would expose the entire dataset, permit session forgery, allow third-party use of the Claude integration, allow reading/altering Square customer + subscription data, allow forging subscription webhooks, or enable service takeover.
