...
- **Subscription/Square (fail closed):** secrets `SQUARE_ACCESS_TOKEN`, `SQUARE_WEBHOOK_SIGNATURE_KEY`; vars `SQUARE_ENVIRONMENT`, `SQUARE_WEBHOOK_URL` (exact public `/api/square/webhook` URL — must byte-match the URL registered in Square; used in HMAC), `SUBSCRIPTION_BYPASS_EMAILS` (always-allowed emails, e.g. owner). Missing → app locked for everyone.
- **In-app checkout:** `SQUARE_LOCATION_ID`, `SQUARE_YEARLY_VARIATION_ID`, `SQUARE_LIFETIME_VARIATION_ID` (plan catalog variation IDs), optional `APP_PUBLIC_URL` (builds `/payment-success` redirect; falls back to `REPLIT_DOMAINS` then request origin). All three plan IDs + `SQUARE_ACCESS_TOKEN` required or `/api/subscription/checkout` returns 503. `SQUARE_STORE_URL` is legacy/optional.
...
- **Subscription/paywall:** backend `lib/squareClient.ts`, `lib/subscriptionService.ts` (`resolveSubscription`), `middlewares/requireSubscription.ts`, `routes/subscription.ts`, schema `lib/db/src/schema/subscriptions.ts`; frontend `SubscriptionGate` in `App.tsx` + `components/checkout-button.tsx`
...
- **Subscription paywall (fail closed).** App is locked until the signed-in user has an ACTIVE Square subscription, matched by **email** (login email = Square payer email). `resolveSubscription()`: owner/allowlist bypass → DB cache (5-min TTL) → live Square check. If Square is unconfigured or errors, access is **denied** (stale "active" never served). `requireSubscription` returns 402 when inactive (skipped only under `NODE_ENV==="test"`). Webhook `POST /api/square/webhook` (HMAC over URL + raw body) refreshes the user's cached row. `SubscriptionGate` sends non-subscribers to `/pricing`.
- **In-app Square checkout.** `POST /api/subscription/checkout` (behind `requireAuth`, **not** `requireSubscription`; body `{planType: yearly|lifetime}`) creates a Square hosted payment link with `buyer_email` = session email and returns `{checkoutUrl}`. 503 if unconfigured, 502 on Square error. Redirects back to `/payment-success`, which re-checks status (entitlement granted async via webhook) before entering `/app`.
...
- **Subscription** — whole app behind a paywall; pick a plan on `/pricing`, pay via in-app Square checkout with your login email, access unlocks once Square reports active
...
- Subscriptions sold via **in-app Square checkout** (hosted payment links) from `/pricing` — no external store page. Identity matched by **email**. Whole app is gated.
...
- **Square webhook signature is over `SQUARE_WEBHOOK_URL` + raw body.** The registered URL must byte-match `SQUARE_WEBHOOK_URL` or verification fails (401). `app.ts` captures `req.rawBody` via the `express.json({ verify })` hook — don't remove it.
- **Subscription is fail closed.** Missing/misconfigured Square secrets lock out everyone except `SUBSCRIPTION_BYPASS_EMAILS` — keep the owner's email there.
- **In-app checkout needs all three plan IDs** or `/api/subscription/checkout` returns 503 (Pricing page still renders).
...
- **Reminder senders are fail-soft** (unlike Square): missing `TWILIO_*` / `RESEND_*` just skips that channel; settings still save.
