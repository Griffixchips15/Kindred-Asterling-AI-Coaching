...
- Required env (already configured): `DATABASE_URL`, `SESSION_SECRET`, `AI_INTEGRATIONS_ANTHROPIC_BASE_URL`, `AI_INTEGRATIONS_ANTHROPIC_API_KEY` (the last two are managed by the Replit Anthropic integration — do not edit). `GEMINI_API_KEY` is no longer used by chat. `ELEVENLABS_API_KEY` powers voice journaling (speech-to-text + Kindred read-aloud); when unset, the voice endpoints return 503 and the UI hides/soft-fails gracefully.
- Reminders env (scheduled SMS + email): SMS via Twilio uses secrets `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_PHONE_NUMBER`; email via Resend uses `RESEND_API_KEY`, `RESEND_FROM_EMAIL`. Each channel is **fail-soft**: when its secrets are unset, that channel reports "not configured" and is simply skipped (the other channel still sends; the UI still lets users save prefs). The Twilio Replit connector proxy was unusable here (error 20003), so both senders use these env secrets directly. **Reminders only fire while the server is running** — a Reserved VM (always-on) deployment is required for the every-minute cron to fire (Autoscale sleeps when idle).
...
- **Voice (backend):** `artifacts/api-server/src/lib/elevenlabs.ts` (ElevenLabs Scribe STT + TTS; `KINDRED_VOICE_ID` = "River"; `isVoiceConfigured()`; `MAX_TTS_CHARS`) and `artifacts/api-server/src/routes/voice.ts` (`POST /voice/transcribe` audio→`{text}`, `POST /voice/speak` `{text}`→`audio/mpeg`) — mounted behind `requireAuth` + `requireSubscription`. These handle **binary media and intentionally bypass OpenAPI/Orval codegen**.
- **Reminders (backend):** `artifacts/api-server/src/lib/reminderScheduler.ts` (every-minute `node-cron` tick; `runReminderTick()` exported for tests; due/window/dedupe logic), senders `src/lib/twilio.ts` (SMS) and `src/lib/resend.ts` (email), routes `src/routes/reminders.ts` (`GET/PUT /reminder-settings`, tagged `profile`). Phone + timezone live on `usersTable` and are edited via the existing profile route. Schema: `lib/db/src/schema/reminders.ts` (`reminder_settings` one-per-user prefs; `reminder_deliveries` idempotency ledger with a unique index on user+type+localDate+doseTime+channel)
- **Reminders (frontend):** `artifacts/kindred-coach/src/pages/reminders.tsx` (channel toggles, phone, auto-detected timezone, per-type enable+time), routed at `/reminders` (Bell icon in the sidebar)
- **Voice (frontend):** `artifacts/kindred-coach/src/lib/voice-api.ts` (direct `fetch` to `/api/voice/*`, same-origin session cookie), `src/hooks/use-voice-recorder.ts` (MediaRecorder dictation), `src/components/voice-input-button.tsx` (mic on journal textareas), `src/components/speak-button.tsx` (read-aloud on Kindred's chat replies)
...
- **Scheduled reminders (cron + idempotency ledger).** A single every-minute `node-cron` task (`reminderScheduler.ts`, started in `index.ts`) computes each user's due reminders **in their own timezone** and sends over their chosen channels (SMS via Twilio, email via Resend). Three correctness guarantees: (1) **Catch-up window** — a reminder fires on the first tick at or after its scheduled local time, up to `WINDOW_MIN` (10) minutes late, so a slow/skipped tick doesn't silently drop a reminder, while long downtime can't replay stale ones. (2) **Atomic once-only delivery** — `deliverOnce()` *reserves* the ledger row first via `INSERT … ON CONFLICT DO NOTHING … RETURNING`; only the winner of that race sends, so overlapping ticks (or multiple processes) can't double-send. A failed send **deletes the reservation** so a later in-window tick can retry. (3) **No overlap / bounded fanout** — a `ticking` guard skips a new tick if the previous one is still running, and the driving query only selects users who have at least one reminder type AND one channel enabled. Medication reminders read `medication_schedule_entries` active that day and group due doses by time. `runReminderTick(now)` is exported and unit-tested (window, dedupe, failed-send retry); the scheduler is skipped entirely under `NODE_ENV==="test"`.
- **Voice journaling (ElevenLabs).** Speech-to-text uses the browser `MediaRecorder` (chosen over the Web Speech API because the latter is unavailable on many mobile browsers, e.g. Brave on Android — the chat composer's old Web Speech dictation was replaced by this) → `POST /api/voice/transcribe` (Scribe `scribe_v1`); the transcript is appended to the active field. Text-to-speech reads Kindred's chat replies aloud via `POST /api/voice/speak` (TTS `eleven_turbo_v2_5`, voice "River"). Because these carry binary audio they **bypass codegen** and the frontend calls them directly with `fetch` (same-origin session cookie). Both are gated by `requireAuth` + `requireSubscription`; with `ELEVENLABS_API_KEY` unset they return 503 and the UI shows a friendly message. In-flight transcribe/speak requests are aborted on stop/unmount (AbortController + mounted guards) so a cancelled read-aloud can't resolve and auto-play.
...
- **Voice** — tap the mic on any journal field (morning, body scan, evening) or the chat box to speak instead of type; tap the speaker on Kindred's chat replies to hear them read aloud in Kindred's voice (powered by ElevenLabs)
- **Reminders** — opt-in scheduled nudges for the morning check-in, medication doses, and the evening reflection, delivered by text and/or email at times you choose in your own timezone (set on the Reminders page)
...
- **Reminders need an always-on server.** The scheduler is an in-process every-minute cron, so it only fires while the API server is running. Deploy on a **Reserved VM** (always-on) — an Autoscale deployment sleeps when idle and reminders won't fire. There is **no** external scheduler.
- **Reminder senders are fail-soft, not fail-closed.** Unlike the subscription gate, missing `TWILIO_*` / `RESEND_*` secrets don't lock anything — that channel just reports "not configured" and is skipped. The Reminders settings page saves fine without them; nudges simply won't send until the secrets are set. (Contrast with Square, which fails closed.)
- **Voice endpoints bypass codegen.** `/api/voice/transcribe` and `/api/voice/speak` carry binary audio, so they are intentionally **not** in `openapi.yaml` and the frontend calls them with raw `fetch` (same-origin cookie). Don't try to regenerate Orval hooks for them. They are gated by `requireAuth` + `requireSubscription`; with `ELEVENLABS_API_KEY` unset they return 503 and the UI soft-fails.
