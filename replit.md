...
- **Multi-dose medications migration:** medications now store a `times` text[] (not a single `time_of_day`) and `medication_logs` carry a `scheduled_time` (unique per user+med+date+time). On a prod DB that still has the old `time_of_day`, a plain `drizzle-kit push` would drop data — run the committed data-preserving migration first: `pnpm --filter @workspace/db run migrate` (applies `lib/db/migrations/0001_multi_dose_medications.sql`, which backfills `times` from `time_of_day` and `scheduled_time` from the owning med, swaps the unique index, then drops the old column). It is idempotent and safe to run before `drizzle-kit push`.
