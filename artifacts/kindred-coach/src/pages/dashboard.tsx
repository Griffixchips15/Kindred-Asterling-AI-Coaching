    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-both">
      <header className="space-y-2 pt-4">
        ...
        <p className="text-muted-foreground text-lg">
          {summary?.currentMentalLoad
            ? `Your mind feels ${summary.currentMentalLoad} right now.`
            : "Take a moment to check in."}
        </p>
      </header>

      {(summaryError || streaksError || moodError) && (
        <QueryErrorState
          title="Dashboard data unavailable"
          message="Kindred could not load all of today's information. Try the request again before adding or changing anything."
          onRetry={() => {
            void Promise.all([
              refetchSummary(),
              refetchStreaks(),
          ...
          <>
            <Card className="border-none shadow-sm bg-primary/5">
              <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-3">
                <div
                  className={`p-3 rounded-full ${summary?.morningDone ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"}`}
                >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {summary?.morningDone ? "Completed" : "Pending"}
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-none shadow-sm bg-secondary/5">
              <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-3">
                <div
                  className={`p-3 rounded-full ${summary?.eveningDone ? "bg-secondary/20 text-secondary" : "bg-muted text-muted-foreground"}`}
                >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {summary?.eveningDone ? "Completed" : "Pending"}
                  </p>
                </div>
              </CardContent>
            </Card>
