...
import { db, morningLogsTable, eveningReportsTable, bodyScansTable } from "@workspace/db";
...
router.get("/weekly-report/pdf", requireAuth, async (req, res): Promise<void> => {
  ...
  res.setHeader("Content-Disposition", 'attachment; filename="kindred-weekly-summary.pdf"');
  ...
  if (eveningReports.length === 0) {
    ...
    for (const report of eveningReports) {
      ...
      entryField("Medication Effectiveness", `${report.medicationEffectiveness} / 10`);
