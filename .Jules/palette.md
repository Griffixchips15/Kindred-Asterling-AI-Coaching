...
## 2024-10-18 - [Add keyboard focus indicators (`focus-visible`) to icon buttons]
**Learning:** Several interactive icon buttons (such as Speak, Voice Input, and Archive in chat, as well as dismissible error messages) lacked visible focus states. Users relying on keyboard navigation couldn't tell which element was currently focused, severely impacting accessibility. In some cases, clickable text like error dismisses were incorrectly implemented using `span` instead of interactive elements like `button`.
**Action:** Always include keyboard focus indicators (`focus-visible:outline-none focus-visible:ring-2`) on all custom interactive elements, and ensure clickable actions are built using semantic `<button>` tags rather than generic text spans.
