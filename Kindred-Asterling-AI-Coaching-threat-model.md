...
Kindred handles high-sensitivity wellness, medication, chat, contact, payment, and calendar data. The leading deployment risks include entitlement identity, webhook state handling, database/network configuration, and real private data committed under `attached_assets/`. The code has tenant scoping, Zod validation, bounded AI/tool payloads, signed webhook handling, and rate limits; these controls materially reduce IDOR, injection, and cost-abuse risk.
...
- Runtime scope: Express 5 API, React/Vite web client, MongoDB, Ollama, ElevenLabs, Helcim/Stripe code, Google Calendar connector, Twilio, Resend, and the reminder scheduler.
...
- Whether production MongoDB is reached only through a private provider network and whether its network allowlist is restricted to Kindred's application hosts.
...
- MongoDB: users, profiles, chat, wellness records, medication state, reminders, and subscription cache (`lib/db/src/mongoSchema.ts`, `lib/db/src/mongoDb.ts`).
...
- Express API -> MongoDB: PII, wellness/medication data, chats, and entitlement cache over the MongoDB driver. Structured filters and pervasive `req.user.id` predicates provide query and tenant controls; the application credential is scoped to one database.
...
flowchart LR
  ...
  API -->|MongoDB TLS| DB["MongoDB replica set"]
